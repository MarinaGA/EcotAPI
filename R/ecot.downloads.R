...
#'
#' @description ecot.downloads makes a serie of request to Ecotopia API based on the choosen type of data, individual and date (optional) for a defined number (optional) of resulting data.
#'
#' @param token a character string giving your token for an open session. For obtaining it, see ecot.token.
#' @param indv_id a character string of the device id from the table of devices information obtained with the API. For obtaining it, see ecot.indvs. The 3 different identifiers (uuid, device number, and device id) that appears on the Ecotopia web are not useful for this.
...
#'
ecot.downloads <- function(token, indv_id, ndevicelimit = 1e3, type = "GPS", maxrounds = NA, datestart_updates = NA, show_count = F){

  if(!is.na(datestart_updates))
    datestart_updates <- gsub(" ","T",paste0(datestart_updates,"Z"))

  Tloop <- ecot.API.request(token,indv_id,ndevicelimit,datestart_updates,type)
  ...
  while(length(Tloop)==ndevicelimit & stop == "NO" & maxrounds != 1){
    ...
    Tloop <- ecot.API.request(token, indv_id, ndevicelimit, to_start, type)
    T_res <- c(T_res,Tloop)
