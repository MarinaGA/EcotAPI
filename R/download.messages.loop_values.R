...
#' @param device_loop parameter already created inside the above mentioned functions
...
#'
#' @return updated values for device_loop, error_count, and Tres  for each round of the loop of download.
#'
#' @export
#'
...
#' @examples
#' \dontrun{
#' download.messages.loop_values(devices_toshow,device_loop,error_count,Tres)
#' }
#' @keywords internal
#'
download.messages.loop_values <- function(devices_toshow,device_loop,error_count,Tres, down_f, token, type, maxrounds, show_count){
tryCatch({

  error_count <- error_count + 1

  cat("\nDevice",device_loop,"-",type,"-",devices_toshow[device_loop],"\n\n") ## this shows the device that is being downloaded
  ...
  assign("device_loop",device_loop,envir = parent.frame())
