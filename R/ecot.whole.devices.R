...
#' @param max_devs the maximun number of to download. Optional.
#'
...
#' @examples
#' \dontrun{
#' T_Env <- ecot.whole.devices("abc","passw", type = "Env", devices = c(11125,11126,11135))
#' }
#'
ecot.whole.devices <-  function(user, psw, token, type = "GPS", devices, maxrounds = NA, show_count = F, max_devs){
    ...
    if(missing(devices))
      stop("You need to provide a vector with the devices number showed on Ecotopia webpage or the UUID to be downloaded.")

    Indv_id <- ecot.indvs(token = token)

    ## if devices provide uuid, this will extract desired devices API ids
    XX <- merge(data.frame(uuid = devices), Indv_id)
    ## if devices provide Ecotopia webpage device number, this will extract desired devices id from the API
    if(nrow(XX)==0)
      XX <- merge(data.frame(mark = devices), Indv_id)

    Indv_id <- XX ## this df should contain only desired devices API ids
    ...
    while(device_loop <= max_devs){
      ...
      if(error_count > nrow(Indv_id)*2){ # the error that motivate the use of tryCatch normally appears once by each whole download.
        print(paste("The indv",device_loop, "produced an error and it was not donwloaded"))
        device_loop <- device_loop + 1
        error_count <- device_loop ## return error_count to the corresponding value of the round
      }
